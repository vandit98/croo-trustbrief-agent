...
5. Open `outputs/demo_report.json`, `outputs/mock_cap_demo.json`, `outputs/requester_demo.json`, `outputs/buyer_composability_demo.json`, `outputs/judge_bundle.json`, and `outputs/dorahacks_demo_package.md`.
6. Point to `claim_assessments`, `source_ledger`, `risk_flags`, `proof.report_hash`, the mock `negotiation_id` -> `order_id` -> `tx_hash` lifecycle, the requester-side schema validation, the buyer-agent correlation ID plus downstream decision, and the bundle's repo plus validation evidence.
7. Show `artifact_freshness.fresh_for_public_demo=true` before presenting the bundle as the current public artifact.
8. Use `outputs/dorahacks_demo_package.md` for the recording sequence, screenshot checklist, source/hash block, A2A buyer proof, and manual live-proof slot.
9. Show `trustbrief_agent/cap_provider.py` handling CROO negotiation and paid delivery.
10. Once dashboard credentials exist, start live provider:
