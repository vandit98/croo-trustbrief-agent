...
6. Open `outputs/demo_report.json`, `outputs/mock_cap_demo.json`, `outputs/requester_demo.json`, `outputs/buyer_composability_demo.json`, `outputs/live_commerce_evidence.json`, `outputs/judge_bundle.json`, and `outputs/dorahacks_demo_package.md`.
7. Point to `claim_assessments`, `source_ledger`, `risk_flags`, `proof.report_hash`, the mock `negotiation_id` -> `order_id` -> `tx_hash` lifecycle, the requester-side schema validation, the buyer-agent correlation ID plus downstream decision, the live-commerce capture slots, and the bundle's repo plus validation evidence.
8. Show `artifact_freshness.fresh_for_public_demo=true` before presenting the bundle as the current public artifact.
9. Use `outputs/dorahacks_demo_package.md` for the recording sequence, screenshot checklist, judge demo capture plan, source/hash block, A2A buyer proof, live-commerce evidence manifest, and manual live-proof slot.
10. Show `trustbrief_agent/cap_provider.py` handling CROO negotiation and paid delivery.
11. Once dashboard credentials exist, start live provider:
